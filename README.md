...
- After ironing out the video requirements, I turned my attention to figuring out what information each of the endpoints would be giving and receiving. The POST endpoints were fairly straightforward, as they provided outlines of the POST data, but the GET requests were a little more opaque, only specifying that a string would be returned in JSON format (`application/json`). Because of this, I had to make some assumptions:
  ...
  - 3. I was at first puzzled by the `video_url` field expected by the `POST /videos` endpoint (what was the point of a video website if the videos were already hosted somewhere else?). Assuming this website was not a redudant collection of already-hosted videos, I had to assume that the videos were stored in some other type of URL, which is how I came to assume that the videos would be stored somewhere as Base64 data URIs. This would solve the problem of determining the video's file type at run time, and avoid the redundant scenario I had considered.
